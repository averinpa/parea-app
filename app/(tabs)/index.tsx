...
const SLIDES = [
  {
    ...
    bg: ['#1a0533', '#0a0814', '#050509'] as [string, string, string],
    accent: '#BB86FC',
    textAccent: '#fff',
  },
  {
    ...
    bg: ['#1a1a35', '#0a0a20', '#050509'] as [string, string, string],
    accent: '#6C63FF',
    textAccent: '#fff',
  },
  {
    ...
    bg: ['#0a2020', '#051515', '#050509'] as [string, string, string],
    accent: '#CCFF00',
    textAccent: '#050509',
  },
]
...
async function isImageSafe(base64: string) {
...
function LandingScreen({ onCreateAccount, onLogin }: { onCreateAccount: () => void; onLogin: () => void }) {
  ...
  const onTouchStart = (e: GestureResponderEvent) => { touchX.current = e.nativeEvent.pageX }
...
function OnboardingScreen({ onFinish }: { onFinish: (data: any) => void }) {
  const TOTAL = 5
...
function FeedScreen() {
  return (
    <LinearGradient colors={['#1a0533', '#050509']} style={[s.fill, s.center]}>
      <StatusBar style="light" />
      <Image source={require('../../assets/images/logo.png')} style={{ width: 160, height: 50 }} resizeMode="contain" />
